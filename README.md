...
POLKA_KEY="<the API key Polka uses to call our webhook>"
```
...
| `POLKA_KEY` | API key the Polka webhook must present (`Authorization: ApiKey <key>`); requests without it are rejected with `401`. |
...
| Method | Path | Auth | Body | Success |
| --- | --- | --- | --- | --- |
| `POST` | `/api/polka/webhooks` | **`ApiKey` (Polka key)** | `{ "event", "data": { "user_id" } }` | `204` |

Polka (the payment provider) calls this when a user subscribes to **Chirpy
Red**. The request must carry the Polka key as `Authorization: ApiKey <key>`
matching `POLKA_KEY`; otherwise the endpoint responds `401`. On a `user.upgraded`
event the user's `is_chirpy_red` is set to `true` and the endpoint returns `204`;
a missing user returns `404`. Any other event is acknowledged with `204` and
ignored. Polka retries on any non-`2XX` response.
